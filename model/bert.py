...
class BertLMPredictionHead(nn.Module):
    ...
    def forward(self, hidden_states, embedings=None, bias=None):
        ...
        if bias is not None and embedings is not None:  # downstream mode
            hidden_states = (
                torch.matmul(hidden_states, embedings.t().to(hidden_states)) + bias
            )
        else:  # MLM mode
            hidden_states = (
                torch.matmul(hidden_states, embedings.t().to(hidden_states))
                + self.bias
            )
        return hidden_states
...
class BertAttention(nn.Module):
    def __init__(self, config, ds_factor, dropout):
        ...
        self.output = BertSelfOutput(config, ds_factor, dropout)
...
class BertOutput(nn.Module):
    def __init__(self, config, ds_factor, dropout, add_moe = False, gating = 'linear'):
        ...
        if not self.add_moe and self.ds_factor:
            ...
            self.moe_layer = MoE(ds_factor=ds_factor, moe_input_size=config.hidden_size, 
                                 dropout=dropout, num_experts=4, top_k=1, gating=gating)
...
class BertLayer(nn.Module):
    def __init__(self, config, ds_factor, ds_factor_ff, dropout, layer_id):
        ...
        self.seq_len_dim = 1
        self.attention = BertAttention(config, ds_factor, dropout)
        ...
        self.output = BertOutput(config, ds_factor_ff, dropout, self.add_moe, gating="linear")
    ...
    def forward(
        ...
        layer_output, loss_moe, load = apply_chunking_to_forward(
        ...
        return outputs, loss_moe, load

    def feed_forward_chunk(self, attention_output, temporal_factor, train_mode):
        intermediate_output = self.intermediate(attention_output)
        layer_output, loss_moe, load = self.output(intermediate_output, attention_output, temporal_factor, train_mode)
...
class BertEncoder(nn.Module):
    def __init__(self, config, ds_factor_attn, ds_factor_ff, dropout):
        ...
        self.layer = nn.ModuleList(
            [BertLayer(config, ds_factor_attn, ds_factor_ff, dropout, _) for _ in range(config.num_hidden_layers)]
        )
    ...
    def forward(
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            layer_outputs, loss_moe_tmp, load = layer_module(
                temporal_factor,
                hidden_states,
                attention_mask,
                layer_head_mask,
...
class BertModel(BertPreTrainedModel):
    ...
    def __init__(
        ...
        self.embeddings = BertEmbeddings(config, features_dim, add_video_feat)
        self.encoder = BertEncoder(config, ds_factor_attn, ds_factor_ff, dropout)
    ...
    def forward(
        ...
        embedding_output, video_g = (
            embedding_output["embeddings"],
            embedding_output["video_global"]
        )

        if video_g is not None:
...
class BertForMaskedLM(BertPreTrainedModel):
    ...
    def forward(
        ...
        if self.n_ans and (not mlm):  # downstream mode
            ...
            embeddings = self.bert.embeddings.word_embeddings.weight
