...
class RobertaEmbeddings(nn.Module):
    ...
    def __init__(self, config, features_dim, add_video_feat = False):
        super().__init__()
            ...
            if self.add_video_feat:
                self.evl = EVLTransformer(10, decoder_num_layers = 1,\
                                          decoder_qkv_dim=768, add_video_feat=self.add_video_feat,\
                                            output_dim=config.hidden_size, add_mask=True)
        # End copy
    ...
    def get_video_embedding(self, video, video_mask):
        if self.add_video_feat:
            ...
            #video = self.evl(video)
            video_feat = self.linear_video(video)
        return None, video_feat
    
    def forward(
        self, input_ids=None, token_type_ids=None, position_ids=None, inputs_embeds=None, past_key_values_length=0, video=None, video_mask=None
    ):
        ...
        if inputs_embeds is None:
            ...
            if self.features_dim and video is not None:
                video_g, video = self.get_video_embedding(video, video_mask)
...
class RobertaAttention(nn.Module):
    def __init__(self, config, ds_factor, dropout, position_embedding_type=None):
        super().__init__()
        self.self = RobertaSelfAttention(config, position_embedding_type=position_embedding_type)
        self.output = RobertaSelfOutput(config, ds_factor, dropout)
...
class RobertaOutput(nn.Module):
    def __init__(self, config, ds_factor, dropout, add_moe):
        super().__init__()
        ...
        if not self.add_moe and self.ds_factor:
            ...
            self.moe_layer = MoE(ds_factor=ds_factor, num_experts=4, top_k=1, dropout=dropout, moe_input_size=config.hidden_size)


    def forward(self, hidden_states, input_tensor, temporal_factor, train_mode) -> torch.Tensor:
        hidden_states = self.dense(hidden_states)
        loss_moe = None
        if not self.add_moe and self.ds_factor:
            hidden_states = self.adapter(hidden_states)
        elif self.add_moe:
            hidden_states, loss_moe, _ = self.moe_layer(temporal_factor, hidden_states, train = train_mode)
        hidden_states = self.dropout(hidden_states)
        hidden_states = self.LayerNorm(hidden_states + input_tensor)
        return hidden_states, loss_moe
class RobertaLayer(nn.Module):
    def __init__(self, config, ds_factor, ds_factor_ff, dropout, layer_id):
        super().__init__()
        ...
        self.attention = RobertaAttention(config, ds_factor, dropout)
        self.intermediate = RobertaIntermediate(config)
        self.output = RobertaOutput(config, ds_factor_ff, dropout, self.add_moe)
    ...
    def forward(
        ...
        layer_output, loss_moe = apply_chunking_to_forward(
            self.feed_forward_chunk, self.chunk_size_feed_forward, self.seq_len_dim, attention_output, temporal_factor, train_mode
        )
        outputs = (layer_output,) + outputs

        # if decoder, return the attn key/values as the last output

        return outputs, loss_moe

    def feed_forward_chunk(self, attention_output, temporal_factor, train_mode):
        intermediate_output = self.intermediate(attention_output)
        layer_output, loss_moe = self.output(intermediate_output, attention_output, temporal_factor, train_mode)
        return layer_output, loss_moe


class RobertaEncoder(nn.Module):
    def __init__(self, config, ds_factor_attn, ds_factor_ff, dropout):
        super().__init__()
        self.config = config
        self.layer = nn.ModuleList([RobertaLayer(config, ds_factor_attn, ds_factor_ff, dropout, _) for _ in range(config.num_hidden_layers)])
    ...
    def forward(
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            layer_outputs, loss_moe_tmp = layer_module(
                temporal_factor,
                hidden_states,
                attention_mask,
                layer_head_mask,
            ...
            if loss_moe_tmp:
                loss_moe = loss_moe + loss_moe_tmp
...
class RobertaModel(RobertaPreTrainedModel):
    ...
    def __init__(
        ...
        self.embeddings = RobertaEmbeddings(config, features_dim, add_video_feat)
        self.encoder = RobertaEncoder(config, ds_factor_attn, ds_factor_ff, dropout)
    ...
    def forward(
        ...
        temporal_factor, embedding_output = embedding_output['video_global'], embedding_output['embeddings']
        if temporal_factor is not None:
            temporal_factor = temporal_factor.squeeze()
...
class RobertaForMaskedLM(RobertaPreTrainedModel):
    ...
    def __init__(
        ...
        self.roberta = RobertaModel(config,
            ...
            freeze_lm=freeze_lm,
            ft_ln=ft_ln,
            ds_factor_attn=ds_factor_attn,
            ds_factor_ff=ds_factor_ff,
            dropout=dropout,
            freeze_ad=freeze_ad,
            add_video_feat=add_video_feat)
    ...
    def set_answer_embeddings(self, a2tok, freeze_last=True):
        a2v = self.roberta.embeddings.word_embeddings(a2tok)
    ...
    def forward(
        ...
        if self.n_ans and (not mlm):  # downstream mode
            ...
            embeddings = self.roberta.embeddings.word_embeddings.weight
        
        prediction_scores = self.lm_head(sequence_output, embeddings, bias)
...
class RobertaLMHead(nn.Module):
    ...
    def forward(self, features, embeddings=None, bias=None):
        x = self.dense(features)
        x = gelu(x)
        x = self.layer_norm(x)

        # project back to size of vocabulary with bias
        if bias is not None and embeddings is not None:  # downstream mode
            hidden_states = (
                torch.matmul(x, embeddings.t().to(x)) + bias
            )
        else:  # MLM mode
            #hidden_states = self.decoder(x)
            
            hidden_states = (
                torch.matmul(x, embeddings.t().to(x))
                + self.bias
            )
