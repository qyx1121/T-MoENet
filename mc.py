...
def train_one_epoch(
    ...
    for i_batch, batch_dict in enumerate(
        ...
        if args.add_video_feat:
            video_mask = torch.cat([torch.ones((video.size(0),1)).to(device),video_mask], dim=1)
        ...
        if data_loader.dataset.mc > 1:
            ...
            cls_loss = (pos_loss + neg_loss) / 2  # balanced BCE
        else:
            cls_loss = F.binary_cross_entropy(logits.squeeze(1), gt.float())

        loss_moe = output["loss_moe"]

        loss_dict = {"cls_loss": cls_loss, "loss_moe":loss_moe}

        if loss_moe != 0:
            loss = cls_loss + loss_moe
        else:
            loss = cls_loss
...
def main(args):
    ...
    for dset_name in args.combine_datasets_val:
        if args.n_ans:
            ...
            if args.distributed:
                model.module.set_answer_embeddings(
